<template>
	<view>
		...
		<view class="title_line" v-for="(listitem,indexs) in list" :key="indexs" :class="{'jiange':indexs%2 ==1}">
			<view class="title_lc">{{listitem.amount}}</view>
			<view class="title_lc title_lc1"  v-if="listitem.status==1">
				處理中
			</view>
			<view class="title_lc title_lc2" v-if="listitem.status==2">
				成功
			</view>
			<view class="title_lc title_lc3" v-if="listitem.status==3">
				失敗
			</view>
			<view class="title_l">
		...
		methods: {
			async getList(callBack) {
				...
				const res = await this.$myRuquest({
					url: '/api/front/user/getRechargeList',
					method: "POST",
					data: {
						page: this.page,
...
<style scoped>
	...
	.title_lc1{ color: #d6bb1f;}
	.title_lc2{ color:  #53D6C4;}
	.title_lc3{ color: #f73131;}
	.value_m {
		border-radius: 50%;
		color: #fff;
		width: 25px;
